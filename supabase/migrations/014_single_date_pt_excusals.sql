create table public.pt_excusal_requests (
  ...
  unique (cadet_id, pt_date)
);

alter table public.pt_excusal_requests enable row level security;

create policy "Users can view accessible dated PT requests"
on public.pt_excusal_requests for select to authenticated
using (cadet_id = (select auth.uid()) or private.is_staff());

create policy "Users can submit own dated PT requests"
on public.pt_excusal_requests for insert to authenticated
with check (
  cadet_id = (select auth.uid())
  and pt_date >= (now() at time zone 'America/New_York')::date
  and status = 'pending'
...
create policy "Staff can review dated PT requests"
on public.pt_excusal_requests for update to authenticated
...
create trigger enforce_dated_pt_review before update on public.pt_excusal_requests
for each row execute function private.enforce_dated_pt_review();

revoke execute on function private.enforce_dated_pt_review() from public, anon, authenticated;
grant select, insert, update on public.pt_excusal_requests to authenticated;
