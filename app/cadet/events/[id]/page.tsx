...
export default async function RequestPage({ params, searchParams }: { params: Promise<{id:string}>, searchParams: Promise<{error?:string}> }) {
  ...
  return <><Nav staff={isStaff} admin={profile.role === 'admin'}/><main className="shell"><section className="hero"><div className="eyebrow">Request Excusal</div><h1 className="h1">{event.name}</h1><p className="sub">{fmtDateTime(event.start_at)}{event.location ? ` · ${event.location}` : ''}</p></section>
