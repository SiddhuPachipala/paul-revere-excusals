...
export function EventFilters({ q, sort, status, eventType, company = 'all', showCompany = false, view }: EventFiltersProps) {
  return (
    <form method="get" className="event-filters">
      ...
      <a className="btn secondary" href={showCompany ? `/staff${view ? `?view=${view}` : ''}` : '/cadet'}>Clear</a>
    </form>
  )
}
